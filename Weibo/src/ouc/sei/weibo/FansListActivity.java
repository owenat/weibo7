 ...
 * 下午3:07:45
 */
public class FansListActivity extends UserActivity {
	...
	public void init() {
		...
		title_text.setText(R.string.fans);
	}
	...
	protected void _onCreate() {
		init();
		this.url = WeiboParameters.Fan_URL+this.getIntent().getExtras().getString("user");
		Logger.v(url);
	...
	protected AdapterView.OnItemClickListener itemClickListener = new AdapterView.OnItemClickListener() {
		public void onItemClick(AdapterView<?> paramAdapterView,
			...
			if ((localObject instanceof FansAdapter)) {
				User User = (User) ((BaseAdapter) localObject)
						.getItem(paramInt - 1);
				Logger.v(((FansAdapter) localObject).getItem(paramInt - 1)
						+ "'");
				Intent localIntent = new Intent(FansListActivity.this,
						UserInfoActivity.class);
				localObject = new Bundle();
				((Bundle) localObject).putSerializable("user", User);
				localIntent.putExtras((Bundle) localObject);
				FansListActivity.this.startActivity(localIntent);
				FansListActivity.this.overridePendingTransition(
